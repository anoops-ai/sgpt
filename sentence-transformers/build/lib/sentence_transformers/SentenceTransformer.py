...
class SentenceTransformer(nn.Sequential):
            ...
            for _ in trange(steps_per_epoch * gradient_accumulation, desc="Iteration", smoothing=0.05, disable=not show_progress_bar):
                for train_idx in range(num_train_objectives):
                    ...
                    if context is not None:
                        det_context.train.report_training_metrics(steps_completed=global_step, {"loss": loss_value})
                ...
                if checkpoint_save_steps is not None and checkpoint_save_steps > 0 \
    ...
    def _eval_during_training(self, evaluator, output_path, save_best_model, epoch, steps, callback, main_process=True, det_context=None):
        ...
        if evaluator is not None:
            ...
            if det_context is not None:
                det_context.report_validation_metrics(steps_completed=step, {"eval_score": score})
